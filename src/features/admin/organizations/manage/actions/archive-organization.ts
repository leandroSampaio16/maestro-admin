...
export async function archiveOrganization(organizationId: string): Promise<{
  success: boolean;
  data?: any;
  error?: string;
}> {
  try {
    ...
    // Prevent archiving the admin organization itself
    if (organizationId === adminOrgId) {
      return {
        success: false,
        error: "Cannot archive the admin organization",
      };
    }
